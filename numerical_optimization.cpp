...
namespace matrixlib {
    ...
    T dot_product(const Matrix<T>& a, const Matrix<T>& b) {
        if(a.size() != b.size() || a.cols() != 1 || b.cols() != 1) {
    ...
    // 共轭梯度法
    template <typename T>
    Matrix<T> conjugate_gradient(const Matrix<T>& A, const Matrix<T>& b, size_t max_iterations, T tolerance) {
        if(A.rows() != A.cols()) {
    ...
    // 雅可比迭代法
    template <typename T>
    Matrix<T> jacobi_iteration(const Matrix<T>& A, const Matrix<T>& b, size_t max_iterations = 1000, T tolerance = 1e-10) {
        if(A.cols() != A.rows()) {
    ...
    // 高斯-赛德尔迭代法
    template <typename T>
    Matrix<T> gauss_seidel_iteration(const Matrix<T>& A, const Matrix<T>& b, size_t max_iterations = 1000, T tolerance = 1e-10) {
        if(A.cols() != A.rows()) {
            std::invalid_argument("Matrix A must be square");
        }
